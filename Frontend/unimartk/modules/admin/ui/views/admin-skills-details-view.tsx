...
export const AdminSkillDetailsView = ({ skillId }: Props) => {
    ...
    const [dialogType, setDialogType] = useState<"reject" | "delete" | null>(null);
    ...
    const deleteSkillMutation = useMutation({
        mutationFn: async ({ skillId, comment }: { skillId: string; comment: string }) => {
    ...
    const approveSkillMutation = useMutation({
        mutationFn: () => {
            // Only send the status field as the API only allows updating status
            const payload = {
                status: "approved"
            };
            
            return approveSkill({ skillId, data: payload });
        },
        onSuccess: () => {
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex flex-row gap-2 mt-4 justify-end items-center w-full p-4 border-t border-border bg-gray-50">
                                ...
                                <Button
                                    ...
                                    disabled={rejectSkillMutation.isPending || data?.status === "rejected"}
                                >
                                ...
                                <Button
                                    ...
                                    disabled={approveSkillMutation.isPending || data?.status === "approved"}
                                >
