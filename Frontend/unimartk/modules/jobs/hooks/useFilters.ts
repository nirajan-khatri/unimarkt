...
const filterParamsParsers = {
  ...
  module: parseAsString.withDefault("").withOptions({
    clearOnDefault: true,
  }),
...
export function useFilters(props?: UseFiltersProps): UseFiltersReturn {
  ...
  // Computed properties
  const computed = {
    hasFilters: !!(minPrice || maxPrice || module),
    hasPriceFilters: !!(minPrice || maxPrice),
    hasLocationFilter: !!module,
    hasCategoryFilters: !!(activeCategory || activeSubcategory),
    hasSearch: !!search,
    hasAnyFilters: !!(minPrice || maxPrice || module || search),
  };
