...
export const useTextModeration = () => {
  ...
  const moderateText = useCallback(
    async (textContent: string): Promise<ModerationResult> => {
      if (!textContent.trim()) {
...
export const useImageModeration = () => {
  ...
  const moderateImage = useCallback(
    async (file: File): Promise<ModerationResult> => {
      setIsLoading(true);
...
export const useContentModeration = () => {
  ...
  const moderateImages = useCallback(
        ...
        for (const file of files) {
          try {
            ...
            if (result.is_inappropriate) {
              warnings.push(
                `Image "${file.name}" contains inappropriate content: ${result.reason}`
              );
              toast.error(
                `Image "${file.name}" was rejected due to inappropriate content.`
              );
            } else {
            ...
            warnings.push(
              `Failed to moderate image "${file.name}". Please try again.`
            );
          }
        }
  ...
  const moderateTextContent = useCallback(
    async (data: {
      name?: string;
      description?: string;
      keyFeatures?: string;
      ...
      try {
        ...
        if (data.name) {
          ...
          if (nameResult.is_inappropriate) {
            warnings.push(
              `Product name contains inappropriate content: ${nameResult.reason}`
            );
        ...
        if (data.description) {
          ...
          if (descResult.is_inappropriate) {
            warnings.push(
              `Product description contains inappropriate content: ${descResult.reason}`
            );
            hasInappropriateContent = true;
          }
        }
        ...
        if (data.keyFeatures) {
          ...
          if (featuresResult.is_inappropriate) {
            warnings.push(
              `Key features contain inappropriate content: ${featuresResult.reason}`
            );
            hasInappropriateContent = true;
          }
        }
        ...
        setState((prev) => ({
          ...
          warnings: [
            ...prev.warnings,
            "Failed to moderate content. Please try again.",
          ],
        }));
        return false;
      } finally {
